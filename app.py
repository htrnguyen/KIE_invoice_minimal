...
def main():
    st.title("Product Information Extraction")
            ...
            if content_file is not None:
                ...
                if submit:
                    ...
                    with col3:
                        ...
                        if "BRAND" in kie_info:
                            st.markdown(f"**Brand:** {kie_info['BRAND']}")

                        if "NAME" in kie_info:
                            st.markdown(f"**Product Name:** {kie_info['NAME']}")

                        if "MFG_LABEL" in kie_info or "MFG" in kie_info:
                            mfg_text = f"{kie_info.get('MFG_LABEL', '')} {kie_info.get('MFG', '')}"
                            st.markdown(f"**Manufacturing Date:** {mfg_text.strip()}")

                        if "EXP_LABEL" in kie_info or "EXP" in kie_info:
                            exp_text = f"{kie_info.get('EXP_LABEL', '')} {kie_info.get('EXP', '')}"
                            st.markdown(f"**Expiration Date:** {exp_text.strip()}")

                        if "WEIGHT_LABEL" in kie_info or "WEIGHT" in kie_info:
                            weight_text = f"{kie_info.get('WEIGHT_LABEL', '')} {kie_info.get('WEIGHT', '')}"
                            st.markdown(f"**Weight:** {weight_text.strip()}")

                        st.markdown("---")
                        show_gpu_info()
                        st.markdown("---")
                        total_runtime = round(float(total_runtime), 4)
                        st.text(f"Total runtime: {total_runtime}s")
